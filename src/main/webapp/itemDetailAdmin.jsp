...
<table>
   ...
   <tr><th>원 산 지</th><td><select name="NATION">
   	<c:forEach var="nation" items="${NATIONS}">
   		<c:choose>
   			<c:when test="${ nation.code == dto.nation }">
   ...
   <tr><th>등 록 일</th><td>%{ dto.reg_date }</td></tr>
     <tr><th>상품설명</th><td><textarea rows="5" cols="40" name="SPEC">${ dto.item_spec}
     </textarea></td></tr>
...
function check(frm){
   if(frm.NAME.value==''){ alert("상품 이름을 입력하세요"); frm.NAME.focus(); return false;}
   if(frm.SPEC.value==''){ alert("상품 설명을 입력하세요"); frm.SPEC.focus(); return false;}
